...
val spark = SparkSession.builder().appName("P5_PCA_Cancer").getOrCreate()

// Use Spark to read in the Cancer_Data file.
val dataset = spark.read.option("header","true").option("inferSchema","true").format("csv").load("Cancer_Data")
...
val assembler = (new VectorAssembler()
                    .setInputCols(colnames).setOutputCol("features"))
...
// and output the pcas features, use 4 principal components
